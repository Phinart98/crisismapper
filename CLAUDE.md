...
**`NUXT_ADMIN_KEY` was retired in Phase 10** — the shared-secret gate is replaced by Supabase
**email+password** staff sessions. `NUXT_PUBLIC_SUPABASE_URL` + `NUXT_PUBLIC_SUPABASE_ANON_KEY` (already
required for Realtime) now also drive auth via `@supabase/ssr`; no new env vars.

**Staff auth (Phase 10):** **email + password** at `/login` (`signInWithPassword`). Accounts are
admin-provisioned (no public signup); a `staff_emails` allowlist does *authorization*. The server
boundary is `server/utils/requireStaff.ts` — it verifies the JWT with **`supabase.auth.getClaims()`**
(local JWKS verification against the project's asymmetric ES256 keys; preferred over `getUser()` which
hits the Auth server every call) → asserts the email ∈ `staff_emails`. Gates `/api/admin/*`,
`/api/translate`, `/api/export`, `/api/auth/me`. RLS (`is_staff()`, the `crisis_reports_authenticated`
view, staff policies on `crises`/`damage_reports`) protects the direct anon-key path. A valid Supabase
account whose email isn't allowlisted is signed out + bounced (`/login?error=not_staff`).

**Why password (not OAuth/magic-link):** the operator has only Gmail + no verifiable sending domain, so
magic-link/SMTP can't deliver (DMARC) and was rate-limited; Google OAuth is the wrong persona (UNDP runs
on **Microsoft 365 / Azure AD**, not Google). Password is universal, needs no email/domain/IdP, and works
offline-of-email. **Production path:** swap `/login` for the org's SSO (UN Azure AD / SAML, which Supabase
supports) behind the *same* `requireStaff` boundary — no other change.

**Staff management UI** (`/admin/staff`, staff-gated): add a staffer (email + initial password → the
`POST /api/admin/staff` route allowlists the email *and* provisions the Supabase login via the
service-role admin API) or remove one (`DELETE` → removes from allowlist + deletes the login; guards
block removing yourself or the last staffer). No SQL needed — operators never touch the DB. Flat model:
any staff can manage staff (role tiers are a future enhancement). `GET /api/admin/staff` lists the
allowlist + a `has_login` flag.

**Demo / judge account** (shipped for evaluation — sandbox, creds are intentionally public):
`demo@crisismapper.app` / `CrisisMapper#Demo2026`. Operator accounts `niiotunarteh@gmail.com` +
`phinart98@gmail.com` share that password (change anytime). All three seeded in `staff_emails`
(`20260601000000_phase10_staff_auth.sql` + the `/admin/staff` console). The very first staffer is
bootstrapped via SQL/service-role; everyone after is added through the UI.

**Operator hardening (free Supabase toggles):** Auth → Providers → Email → **disable "Enable Signup"**
(only admin-provisioned accounts; the allowlist gates regardless); set a **password policy** (≥8 + mixed
chars). Asymmetric JWT keys are already on (free, default for this project) so `getClaims()` verifies
locally. *Leaked-password protection is Pro-only — skip unless upgrading.*
...
by default (`?include_duplicates=true` to keep dupes). Staff-session gated (exact data = staff data
per Q&A #18 — Phase 10 retired the interim admin key). Duplicate detection, `quality_score`, and reporter
